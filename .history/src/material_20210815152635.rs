    ...
    pub fn scatter(&self, r_in: &Ray, rec: &HitRecord) -> (Ray, Vec3, bool) {
        match self {
            Material::Lambertian { albedo } => {
                ...
                (Ray::new(rec.p, scatter_direction, r_in.time), *albedo, true)
            },
            Material::Metal { albedo, roughness } => {
                 let reflected = reflect((r_in.direction).normalized(), rec.normal);
                 let scattered = Ray::new(rec.p, reflected + (*roughness) * random_in_unit_sphere(), r_in.time);
            ...
            Material::Dielectric { index_refraction } => {
                ...
                let scattered = match refract(r_in.direction, outward_normal, ni_over_nt) {
                    ...
                    None => Ray::new(rec.p, reflected, r_in.time)
                };
