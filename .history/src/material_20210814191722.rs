...
pub enum Material {
    Lambertian { albedo: Vec3x8 },
    Metal { albedo: Vec3x8, roughness: f32 },
    Dielectric { index_refraction: f32 }
}

impl Material {
    pub fn scatter(&self, r_in: &Ray, rec: &HitRecord) -> (Ray, Vec3x8, bool) {
        match self {
            ...
            Material::Metal { albedo, roughness } => {
                 ...
                 let b = scattered.direction.dot(rec.normal) > 0.0;
            ...
            Material::Dielectric { index_refraction } => {
                let (outward_normal, ni_over_nt, cosine) = if r_in.direction.dot(rec.normal) > 0.0 {
                    (
                        -rec.normal,
                        *index_refraction,
                        *index_refraction * r_in.direction.dot(rec.normal) / r_in.direction.mag()
                    )
                } else {
                    (
                        rec.normal,
                        1.0 / *index_refraction,
                        -(r_in.direction.dot(rec.normal)) / r_in.direction.mag()
                    )
                };
                ...
                let attenuation = Vec3x8::new(1.0, 1.0, 1.0);
...
pub fn reflect(v1: Vec3x8, v2: Vec3x8) -> Vec3x8 {
    return v1 - 2.0 * v1.dot(v2) * v2
}

pub fn refract(v: Vec3x8, n: Vec3x8, ni_over_nt: f32) -> Option<Vec3x8> {
    let uv = (v).normalized();
    let dt = uv.dot(n);
